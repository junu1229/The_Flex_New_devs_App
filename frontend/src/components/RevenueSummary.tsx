...
import { SecureAPI, type DashboardSummaryResponse } from '../lib/secureApi';
...
const FIXED_TWO_DECIMAL_PATTERN = /^([+-]?)(\d+)\.(\d{2})$/;

function formatFixedTwoDecimal(value: string): string | null {
    const match = FIXED_TWO_DECIMAL_PATTERN.exec(value);
    if (!match) return null;

    const [, sign, integerDigits, cents] = match;
    return `${sign}${integerDigits.replace(/\B(?=(\d{3})+(?!\d))/g, ',')}.${cents}`;
}
...
export const RevenueSummary: React.FC<RevenueSummaryProps> = ({ propertyId, year, month, showRaw }) => {
    ...
    const displayTotal = formatFixedTwoDecimal(data.total_revenue);
    if (displayTotal === null) {
        return <div className="p-4 text-red-500 bg-red-50 rounded-lg">Invalid revenue data</div>;
    }
